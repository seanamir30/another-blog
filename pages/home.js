    ...
    return (
        <div className="xl:px-[30rem] md:px-24 px-2 py-7 xl:py-16 text-white">
            ...
            {posts && posts.map((post)=>{
                if (!post) return
                return(
                    <div onClick={()=>Router.push(`/view/${post.id}`)} className="relative cursor-pointer max-h-48 bg-slate-800 w-full text-white border-2 border-slate-700 hover:border-slate-400 outline-none mt-3 p-3 rounded-md" key={post.id}>
